...
#===============================================================================
# Creating the first loop to implement the decisions for the current week, lead times, 
# penalty and cost
...
for (j in 1:52){
  ...
  # 2nd Approach: Calculate the number of operations and p-sessions required for j+2 (in 2 weeks)
  # provide exactly the number of operations and p-sessions needed, as long as there is enough capacity
  # prioritize the operations because they have the highest penalty
  # Calculate the number of p-sessions needed and execute them, 
  # if capacity after scheduling of the operations is available 
  # sessions will be scheduled 2 weeks in advance:
  
  D3M$os_2[j+2] <- min(ceiling(( D3M$O[j+2] +  LEAD$P1_O[1] *  min(D3M$O[j+1], D3M$os_0[j+1] * o_sessions) + LEAD$P2_O[1] * min(D3M$P2[j+1], D3M$ps_0[j+1] * p_sessions - D3M$Treated_P1[j+1])) /3 ), D3M$capacity[j+2])
  
  D3M$ps_2[j+2] <- min(ceiling(( D3M$P1[j+2] + (D3M$P1[j+1] - min(D3M$P1[j+1], D3M$ps_0[j+1] * p_sessions)) + 
                                   (D3M$P2[j+2] + (D3M$P2[j+1] - min(D3M$P2[j+1], D3M$ps_0[j+1] * p_sessions - D3M$Treated_P1[j+1])) +
                                      LEAD$P1_P2[1] * min(D3M$P1[j+1], D3M$ps_0[j+1] * p_sessions) + 
                                      LEAD$O_P2[1] * min(D3M$O[j+1], D3M$os_0[j+1] * o_sessions)) + 
                                   (D3M$P3[j+2] + (D3M$P3[j+1] - min(D3M$P3[j+1], D3M$ps_0[j+1] * p_sessions - D3M$Treated_P1[j+1] - min(D3M$P2[j+1], D3M$ps_0[j+1] * p_sessions - D3M$Treated_P1[j+1]))) + 
                                      LEAD$P2_P3[1] * min(D3M$P2[j+1], D3M$ps_0[j+1] * p_sessions - D3M$Treated_P1[j+1]) + 
                                      LEAD$P3_P3[1] * min(D3M$P3[j+1], D3M$ps_0[j+1] * p_sessions - D3M$Treated_P1[j+1] - min(D3M$P2[j+1], D3M$ps_0[j+1] * p_sessions - D3M$Treated_P1[j+1])))) /16 ), D3M$capacity[j+2] - D3M$os_2[j+2])

  D3M$os_0[j+2] <- D3M$os_2[j+2]
  D3M$ps_0[j+2] <- D3M$ps_2[j+2]
  
}

#===============================================================================
# Creating a second loop to implement the delays of O, P1, P2, P3 and the costs 
...
write_xlsx(D3M, "D3M_Solution_1st_Approach.xlsx")
